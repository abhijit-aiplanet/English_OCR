...
# Configure CORS - Allow Vercel deployment origins
...
# Configure Gemini API Key from Vercel environment variables
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
if not GEMINI_API_KEY:
    print("⚠ Warning: GEMINI_API_KEY not found in environment variables")
    print("Please set it in Vercel project settings")
else:
    print(f"✓ Gemini API Key loaded (length: {len(GEMINI_API_KEY)} chars)")
...
async def process_image_with_gemini(image: Image.Image, page_number: int) -> str:
    ...
    """
    if not client:
        raise HTTPException(status_code=500, detail="Gemini API client not initialized. Please configure GEMINI_API_KEY.")
...
@app.get("/")
async def root():
    return {"message": "Handwritten Form OCR API", "status": "running", "platform": "Vercel"}
...
async def perform_ocr(file: UploadFile = File(...)):
    ...
    Note: Use /api/ocr/stream for real-time results as pages are processed.
    """
    # Validate file type
    if not file.filename.lower().endswith('.pdf'):
        raise HTTPException(status_code=400, detail="Only PDF files are allowed")
...
async def health_check():
    ...
    return {"status": "healthy", "gemini_configured": bool(GEMINI_API_KEY), "platform": "Vercel"}
